...
def build_dynamic_graphs(processed_csv_path, metadata_json_path, output_dir, window_size=256, stride=128):
    ...
    windows_H = [] # Hypergraph incidence matrices [Windows, N, N]
    windows_W = [] # Hyperedge weights [Windows, N]
    window_timestamps = []
    ...
    for start_idx in range(0, n_steps - window_size + 1, stride):
        ...
        H, W_e = build_hypergraph(A, window_size, app_states)
            
        windows_X.append(X)
        windows_A.append(A)
        windows_H.append(H)
        windows_W.append(W_e)
    ...
    tensor_H = torch.tensor(np.array(windows_H), dtype=torch.float32)
    tensor_W = torch.tensor(np.array(windows_W), dtype=torch.float32)
