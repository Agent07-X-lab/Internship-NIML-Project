...
def run_predictive_maintenance_pipeline(input_path, output_dir="3_processed_outputs", epochs=15):
    ...
    # Run original refit preprocessor if not already processed
    if "processed" in os.path.basename(input_path).lower():
        print(f"Input file is already standardized processed CSV. Skipping preprocessor.")
        processed_csv = input_path
        metadata_json = os.path.join(output_dir, f"House_{house_id}_Metadata.json")
        if not os.path.exists(metadata_json):
            # Write a fallback metadata JSON for house 99 if missing
            fallback_meta = {
                "House": house_id,
                ...
                "EndTime": "2026-07-13 06:40:00",
                "SamplingFrequency": "8s",
                "Mappings": {f"Appliance{i}": name for i, name in enumerate(["Washing_Machine", "Fridge_Freezer", "Kettle", "Dishwasher", "Microwave", "Oven_Extractor_Fan", "Television", "Tumble_Dryer", "Toaster"], 1)},
                "Thresholds": {"Washing_Machine": 20.0, "Fridge_Freezer": 15.0, "Kettle": 1000.0, "Dishwasher": 50.0, "Microwave": 100.0, "Oven_Extractor_Fan": 50.0, "Television": 10.0, "Tumble_Dryer": 20.0, "Toaster": 100.0}
            }
    ...
    H = data["H"] if "H" in data else torch.zeros(X.size(0), len(appliance_names), len(appliance_names))
    W_e = data["W_e"] if "W_e" in data else torch.zeros(X.size(0), len(appliance_names))
    timestamps = data["timestamps"]
    mean_A = data["mean_A"]
    mean_H = H.mean(dim=0).numpy()
    mean_W = W_e.mean(dim=0).numpy()
    ...
    A_train = A[:split_idx]
    H_train = H[:split_idx]
    W_train = W_e[:split_idx]
    X_test = X[split_idx:]
    A_test = A[split_idx:]
    H_test = H[split_idx:]
    W_test = W_e[split_idx:]
    ...
    if not model_exists and epochs > 0:
        ...
        train_loader = DataLoader(TensorDataset(X_train, A_train_gate, H_train, W_train), batch_size=16, shuffle=True)
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_X, batch_A, batch_H, batch_W in train_loader:
                optimizer.zero_grad()
                X_recon, A_recon, H_recon, _, _ = model(batch_X, batch_A, batch_H, batch_W)
                loss_X = mse_fn(X_recon, batch_X)
                loss_G = bce_fn(A_recon, batch_A)
                # target H should be scaled appropriately or we use MSE for H_recon depending on definition, using BCE since it's incidence
                loss_H = bce_fn(H_recon, batch_H)
                
                loss = loss_X + 0.2 * loss_G + 0.1 * loss_H
                loss.backward()
                optimizer.step()
    ...
    mean_A_numpy = mean_A if isinstance(mean_A, np.ndarray) else mean_A.numpy()  # shape [4, N, N], channel 0 = Jaccard
    ...
    baseline_thresholds = {}
    with torch.no_grad():
        X_train_recon, _, _, _, _ = model(X_train, A_train_gate, H_train, W_train)
    ...
    with torch.no_grad():
        X_test_recon, A_test_recon, H_test_recon, A_final_n, attn_weights_n = model(X_test, A_test_gate, H_test, W_test)
        X_faulty_recon, A_faulty_recon, H_faulty_recon, A_final_f, attn_weights_f = model(X_test_faulty, A_test_gate, H_test, W_test)
        
        # Hypergraph drift for the whole system
        final_hypergraph_drift = calculate_hypergraph_drift(H_test[-1].numpy(), W_test[-1].numpy(), mean_H, mean_W)
        ...
        for i, name in enumerate(appliance_names):
            ...
            # Anomaly Score
            score = calculate_anomaly_score(faulty_mse, final_drift_score, final_hypergraph_drift)
            ...
            attn = attn_weights_f[-1, i].numpy().tolist()
            ...
            appliance_analytics.append({
                ...
                "name": name,
                "icon": get_icon(name),
                "reconstruction_error": float(faulty_mse),
                "graph_drift": float(final_drift_score),
                "hypergraph_drift": float(final_hypergraph_drift),
                "anomaly_score": float(score),
                "health": max(0.0, 100.0 - float(score) * 50.0), # Mock health for legacy UI support
                "rul": 90, # Mock RUL for legacy UI support
                "fault_status": severity,
                "severity": severity,
                "fault_type": root_cause,
                "root_cause": root_cause,
                "before_fault_signal": base_power_val.tolist(),
                "after_fault_signal": power_val.tolist(),
                "attention_weights": attn,
                "important_features": ["Power", "Duration"],
    ...
    template_path = os.path.join(ROOT_DIR, "web_dashboard", "templates", "dashboard_pm_template.html")
    ...
    root_out_path = os.path.join(ROOT_DIR, "Predictive_Maintenance_Dashboard.html")
...
def main():
    ...
    parser.add_argument("--output-dir", default=os.path.join(ROOT_DIR, "3_processed_outputs"), help="Output directory")
    args = parser.parse_args()
    
    raw_dir = os.path.join(ROOT_DIR, "1_raw_data")
